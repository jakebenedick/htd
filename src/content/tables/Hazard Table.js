	...
	const createHeadCells = (headerArray) => {
		...
		for (i = 0; i < headerArray.length; i++) {
			headers.push({
				...
				id:
					'hazard_' +
					headerArray[i].replaceAll(' ', '_').toLowerCase(),
			});
		}
	...
	return (
		<Toolbar
			...
			{numSelected > 0 ? (
				...
				<Tooltip title='Export Search Results'>
					<IconButton
...
function EnhancedTable(props) {
	...
	const onDeleteButtonClicked = () => {
		for (let i = 0; i < selected.length; i++) {
			axios
				.delete(`http://localhost:4000/hazard/delete/${selected[i]}`)
				.then((res) => {
					if (res.status === 200) {
	...
	const handleSelectAllClick = (event) => {
		if (event.target.checked) {
			const newSelecteds = rows.map((n) => n.hazard_id);
	...
	const handleClick = (event, hazard_id) => {
		const selectedIndex = selected.indexOf(hazard_id);
		let newSelected = [];

		if (selectedIndex === -1) {
			newSelected = newSelected.concat(selected, hazard_id);
	...
	*/
	...
	const updateFunction = (event, row) => {
		if (user.level !== 'Admin') {
			...
			history.push(`/edit/hazard/${hazard_id}`);
		}
	};
								...
								.map((row, index) => {
									...
									return (
										<TableRow
											...
											<TableCell>
												{row.hazard_supporting_document}
											</TableCell>
											{/* <TableCell>
												{row.hazard_sss_list}
											</TableCell> */}
