...
function EnhancedTableHead(props) {
	...
	const createHeadCells = (headerArray) => {
		...
		for (i = 0; i < headerArray.length; i++) {
			headers.push({
				label: headerArray[i],
				id: 'ssd_' + headerArray[i].replaceAll(' ', '_').toLowerCase(),
			});
		}
...
const EnhancedTableToolbar = (props) => {
	...
	const exportToPdf = (data, title, category, searchCriteria) => {
		const headers = [
			[
				'SSD ID',
				'SSD Requirement Text',
				'SSD Safety Control List',
				'SSD Status',
				'Mapped SSS',
				'SSD Program',
			],
		...
		const dataForExport = data.map((entry) => [
			...
			entry.ssd_safety_control_list,
			entry.ssd_status ? 'Passed' : 'Not Passed',
...
function EnhancedTable(props) {
	...
	const onDeleteButtonClicked = () => {
		if (user.level !== 'Admin') {
			...
			for (let i = 0; i < selected.length; i++) {
				axios
					.delete(`http://localhost:4000/ssd/delete/${selected[i]}`)
					.then((res) => {
						if (res.status === 200) {
	...
	const handleSelectAllClick = (event) => {
		if (event.target.checked) {
			const newSelecteds = rows.map((n) => n.ssd_id);
	...
	const handleClick = (event, ssd_id) => {
		const selectedIndex = selected.indexOf(ssd_id);
		let newSelected = [];

		if (selectedIndex === -1) {
			newSelected = newSelected.concat(selected, ssd_id);
	...
	const splitSSSMapping = (mappingString) => {
	...
	*/
	...
	const updateFunction = (event, row) => {
		if (user.level !== 'Admin') {
			...
			history.push(`/edit/ssd/${ssd_id}`);
		}
	};

	const isSelected = (ssd_id) => selected.indexOf(ssd_id) !== -1;
									...
									return (
										<TableRow
											...
											<TableCell
												...
												padding='none'>
												<Link
													href={`/Mapping/SSD/${row.ssd_program}/${row.ssd_id}`}>
													{row.ssd_id}
												</Link>
											</TableCell>
											<TableCell>
												{row.ssd_requirement_text}
											</TableCell>
											<TableCell>
												{row.ssd_status
													? 'PASSED'
											...
											<TableCell>
												{splitSSSMapping(
													...
													<Link
														href={`/Mapping/SSS/${row.ssd_program}/${value}`}>
														{value}
														<br></br>
													</Link>
												))}
